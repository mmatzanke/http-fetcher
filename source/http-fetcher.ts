...
export type RequestOptions = Pick<
  Options,
  "body" | "credentials" | "headers" | "searchParams" | "signal" | "timeout"
>;
...
type ParametersWithoutPayload<T extends HttpFetcherOptions> =
  T["baseUrl"] extends URL
    ? ParametersWithoutPayloadBase & ParametersWithPathname
    : ParametersWithoutPayloadBase & ParametersWithUrl;

type ParametersWithJsonPayload<T extends HttpFetcherOptions> =
  ParametersWithoutPayload<T> & {
    readonly payload: unknown;
  };
...
export type HttpFetcherError = Error & {
...
export interface HttpFetcher<
  T extends HttpFetcherOptions = HttpFetcherOptions
> {
  ...
  return {
    ...
    set requestHeaders(_requestHeaders: RequestHeaders) {
      requestHeaders = requestHeaders.mapOrElse<MaybeType<RequestHeaders>>(
        () => MaybeModule.of(_requestHeaders),
        (headers: RequestHeaders) =>
          MaybeModule.of<RequestHeaders>({
            ...headers,
            ..._requestHeaders,
          })
      );
